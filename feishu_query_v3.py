...
用法:
    python3 feishu_query_v3.py           # 只查询未签收的记录
    python3 feishu_query_v3.py --force   # 查询所有记录
    python3 feishu_query_v3.py --limit 5 # 只查询前 5 条
    python3 feishu_query_v3.py --workers 20  # 设置并发线程数（默认 10）
"""
...
def process_single(item):
    ...
    if not tracking_list:
        safe_print(f"[{idx}/{total}] {order_id} ... ⚠️  无数据")
        return item["record_id"], None
...
def main():
    parser = argparse.ArgumentParser(description="飞书多维表格物流查询工具（并发版）")
    ...
    try:
        ...
        with ThreadPoolExecutor(max_workers=workers) as executor:
            futures = {executor.submit(process_single, item): item for item in to_query}
